...
async fn handle_socket(socket: WebSocket, state: Arc<AppState>) {
    ...
    let send_task = tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            ...
            if msg.encode(&mut buf).is_ok()
                && sender.send(Message::Binary(buf.into())).await.is_err()
            {
                break;
            }
        }
    ...
    while let Some(Ok(msg)) = receiver.next().await {
        let sig_msg: SignalingMessage = match msg {
            Message::Binary(bin) => match SignalingMessage::decode(bin) {
                Ok(m) => m,
                Err(_) => continue,
            },
            _ => continue,
        ...
        if let Some(content) = sig_msg.content {
            handle_signaling_message(&state, content, &tx, &mut current_device_id).await;
        }
    }
...
async fn handle_signaling_message(
    ...
    match content {
        Content::Init(init) => {
            ...
            let _permit =
                match tokio::time::timeout(handshake_timeout, state.handshake_semaphore.acquire())
                    .await
                {
                    Ok(Ok(p)) => p,
                    _ => {
                        tracing::warn!(
                            "Handshake timeout or throttle active for {}",
                            init.device_id
                        );
                        let _ = tx
                            .send(SignalingMessage {
            ...
            let msg_to_sign = format!(
                "WS_INIT:{}:{}:{}",
                init.device_id, init.identity_id, init.timestamp
            );
            ...
            let sig_verified = match tokio::time::timeout(
                handshake_timeout,
