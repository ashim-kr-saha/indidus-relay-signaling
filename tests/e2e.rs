...
async fn test_full_lifecycle_v4() {
    ...
    let res = client.post(server.url("/register"))
        .json(&json!({
            "username": username,
    ...
    let res = client.post(server.url("/shares"))
    ...
    // 4. Download
    let res = client.get(server.url(&format!("/shares/{}", share_id)))
        .send().await.unwrap();
    ...
    let res = client.delete(server.url(&format!("/shares/{}", share_id)))
        .header("X-Identity", username)
        .header("X-Public-Key", &public_key_hex)
        .header("X-Timestamp", timestamp.to_string())
        .header("X-Signature", signature)
