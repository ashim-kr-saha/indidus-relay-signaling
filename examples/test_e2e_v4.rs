...
fn main() {
    ...
    let res = client
        .get(format!("{}/shares/{}", base_url, share_id))
        .send()
        .expect("Failed to download share");
    ...
    let res = client
        .get(format!("{}/v/{}", base_url, share_id))
        .send()
    ...
    let res = client
        .get(format!("{}/pkg/indidus_wasm_share_client.js", base_url))
        .send()
        .expect("Failed to get JS asset");
    if res.status() == 200 {
        println!("Viewer JS asset route works!");
    } else {
        println!(
            "Viewer JS asset route returned {}, (this is expected if wasm is not yet built in assets/pkg/)",
            res.status()
        );
    }
    ...
    let res = client
        .delete(format!("{}/shares/{}", base_url, share_id))
        .header("X-Identity", &username)
        .header("X-Public-Key", &public_key_hex)
        .header("X-Timestamp", timestamp.to_string())
        .header("X-Signature", signature)
    ...
    let res = client
        .get(format!("{}/shares/{}", base_url, share_id))
        .send()
        .expect("Failed to check deleted share");
